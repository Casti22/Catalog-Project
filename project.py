...
# Store it in the session for later valida
@app.route('/login')
def showLogin():
...
def gconnect():
  ...
  url  = (
    'https://googleapis.com/oauth2/v1/tokeninfo?access_token=%s'
    % access_token)
  h = httplib2.Http()
  result = json.loads(h.request(url, 'GET')[1])

  # If there was an error in the access token info, abort.
  if reslut.get('error') is not None:
    response = make_response(json.dumps(
